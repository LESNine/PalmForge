...
#### Option 1: Standalone Package (No Node.js Required)

1. Download `PalmForge_v1.0.zip` from the latest release
2. Extract to any folder
3. Double-click `start.bat` — a local HTTP server will start and open your browser automatically
4. Press **Q** in the terminal window to stop the server

> ⚠️ Do NOT open `index.html` directly — browsers block ES modules from `file://` protocol. Always use `start.bat`.

#### Option 2: Developer Package (Requires Node.js)

1. Download `PalmForge_Full_v1.0.zip` from the latest release
2. Extract to any folder
3. Make sure [Node.js](https://nodejs.org/) (LTS) is installed
4. Double-click `start.bat` — it will check Node.js, install dependencies if needed, and launch the dev server
...
1. 下载最新 release 中的 `PalmForge_v1.0.zip`
2. 解压至任意文件夹
3. 双击 `start.bat` — 程序会启动本地 HTTP 服务器并自动打开浏览器
4. 在终端窗口按 **Q** 键即可关闭服务器

> ⚠️ 请勿直接双击 `index.html` 打开——浏览器会因安全策略显示空白页，必须通过 `start.bat` 启动。

#### 方式二：完整开发版（需要 Node.js）

1. 下载最新 release 中的 `PalmForge_Full_v1.0.zip`
2. 解压至任意文件夹
3. 确保已安装 [Node.js](https://nodejs.org/)（推荐 LTS 版本）
4. 双击 `start.bat` — 程序会自动检测 Node.js、安装依赖并启动开发服务器
